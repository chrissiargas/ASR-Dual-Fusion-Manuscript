...
% REVIEW: (1) removed the claim that the gate protects "purely syntactic tokens": a single
% scalar per layer cannot act per token. (2) From the code: queries are layer-normalized,
% attention dropout is 0.2, projections have no bias, and there is no FFN (unlike Flamingo);
% this matches the 59M/132M counts. (3) Report the learned tanh(g) values per layer.
...
% REVIEW: (1) the Greek backbone is an Instruct model while Racka-4B is not stated as such
% -- justify or align. (2) The two languages use LLMs of different sizes, so do not draw
% cross-language conclusions about the method.
...
%TODO scan the 4 internal datasets and provide information on their size, domain, and nature to assure reviewers they aren't the sole drivers of your performance gains.
% (6) Cite the exact YODAS release. If you used a pseudo-labelled release (e.g. the
%     Granary version of YODAS2), cite that release instead of li2023yodas.
...
% REVIEW: (1) The corpus-specific style prompt gives the model oracle domain information
% that zero-shot baselines do not receive; acknowledge it, or report results with a
% single default prompt. (2) A repetition penalty can penalize legitimate repeated words
% in verbatim speech; justify it. (3) Report real-time factor / latency vs. Whisper: an
% 8B LLM with beam 4 is far more expensive, and ICASSP reviewers will ask.
...
% REVIEW: (1) state the exact normalizer: Whisper's EnglishTextNormalizer must not be
% used for Greek/Hungarian; with BasicTextNormalizer, state whether diacritics are removed
% (removing them is invalid for Hungarian ö/ő/o and changes Greek tonos handling).
% (2) REMOVED the claim that good results on both metrics show the model "predicts
% syntactic boundaries from acoustic prosody": the LLM text prior alone can produce
% punctuation. To claim it, report punctuation F1 and an audio-vs-text-only comparison.
% (3) Add significance testing (e.g., bootstrap confidence intervals or MAPSSWE),
% especially for the small FLEURS test sets.
...
\midrule
Common Voice(19h)~\cite{ardila2020commonvoice}      & 7.3 / \underline{5.1} / 3.0 & \textcolor{win}{-4.74} & 12.04 & 13.98 \\
FLEURS(12h)~\cite{conneau2022fleurs}          & 10.9 / 9.9 / 4.8 & \textcolor{loss}{+1.57} & \underline{9.3}  & 11.02 \\
HParl(120h)~\cite{paraskevopoulos2023hparl}            & 8.2 / \underline{8.2} / 4.7 & \textcolor{win}{-4.81} & 13.01 & 16.99 \\
Logotypographia(61h)~\cite{digalakis2003logotypographia}  & 6.6 / \underline{6.6} / 2.0 & \textcolor{win}{-2.80} & 9.4   & 10.86 \\
TEDx(29h)~\cite{salesky2021mtedx}             & 17.8 / 13.9 / 9.5 & -- & -- & \underline{12.3}    \\
\bottomrule
\end{tabular}%
}
\end{table}
...
public Hungarian corpora against Whisper large-v3. We deliberately exclude the Whisper-Hu~\cite{trendency2024whisperlargev3hu} finetuned model due to disclosed test-set contamination on VoxPopuli, Common Voice, and Speech-Massive. Our model outperforms Whisper on four of the five comparable corpora, with the largest margins on Common Voice ($\Delta = -7.1$). The sole loss is Yodas, where plain Whisper remains stronger ($11.3$ vs. $6.2$, $\Delta = +5.1$). This loss likely reflects reference provenance rather than a real shortfall: Yodas transcripts are pseudo-labeled by faster-whisper-large-v3 itself, so comparing us to zero-shot Whisper on Whisper-generated references is close to self-referential evaluation.
Crucially, to isolate our architectural improvements from the proprietary data advantage, Table~\ref{tab:public_data_ablation} details the performance of a model variant trained exclusively on the public corpora. Even without the 1,500-hour internal dataset, the public-only model suffers only a marginal degradation, maintaining a strict advantage over the Whisper baseline across the four comparable datasets and confirming the architectural efficacy of Dual-Fusion.
...
\begin{table}[h]
    ...
    \begin{tabular}{l | c c | c }
        ...
        \midrule
        Common Voice(126h)~\cite{ardila2020commonvoice}      & 7.4 / \underline{6.0} / 2.0  & \textcolor{win}{-7.1}  & 13.1 \\
        FLEURS(14h)~\cite{conneau2022fleurs}              & 10.4 / \underline{7.7} / 2.9 & \textcolor{win}{-5.7} & 13.4 \\
        Speech Massive(6h)~\cite{lee2024speechmassive}       & 32.2 / \underline{14.1} / 8.1 & \textcolor{win}{-5.3} & 19.4 \\
        VoxPopuli(60h)~\cite{wang2021voxpopuli}            & 10.0 / \underline{7.8} / 4.9  & \textcolor{win}{-6.7}  & 14.5 \\
        Yodas(114h)~\cite{li2023yodas}                & 14.0 / 11.3 / 7.1 & \textcolor{loss}{+5.1} & \underline{6.2}  \\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
\begin{table}[h]
    \centering
    \caption{Impact of proprietary data on n-WER (\%): \textbf{Ours (Public)} represents the model trained exclusively on public corpora. $\Delta$ indicates the error rate degradation when the internal datasets have been removed}
...
%TODO mention that internal datasets are used for training the full model. Explain the duration, and the domain of these datasets to assure reviewers they aren't the sole drivers of your performance gains. The durations are: 1) Internal-1: 1,500 hours of Read Speech and Spontaneous / Conversational data, 2) Internal-2: 2000 hours of Conversational data. These datasets are proprietary and not publicly available, but they are used to enhance the model's performance on medium-resource hungarian.
...
%misspells foreign names
%the encoder's fixed 30\,s window truncates longer inputs
%
...
Finally, our evaluation is also limited to two medium-resource languages, a single frozen acoustic encoder (Whisper-large-v3); generalization to other languages, encoders, or truly low-resource settings remains untested.

\subsection{Ablation Study}
\label{subsec:ablation}
\noindent
To isolate component contributions without prohibitive training costs, we evaluate ablations on the validation set at 12k steps (Table~\ref{tab:ablation}).
As shown in Table~\ref{tab:ablation}, the Full Model achieves a baseline validation WER of $21.4$\%. Removing or altering any core component consistently degrades performance, validating our design:
(i) removing the prefix audio tokens causes the largest loss ($+3.3$). It deprives the LLM's early layers of global acoustic context. Most importantly, prefix tokens are visible to self-attention in every block from the first step;
(ii) using only the final encoder layer degrades WER by $1.8$, as intermediate layers retain phonetic detail~\cite{yang2021superbspeechprocessinguniversal}. Static, utterance-independent layer weights recover part of this gap but still degrades by $+1.1$, showing that adapting the mixture to each utterance brings a further gain;
(iii) removing the zero-initialized $\tanh$ gate ($+1.1$) injects unstructured, randomly-initialized noise into the pre-trained LLM from the first update. The gate ensures stable, gradual integration of the acoustic signal;
(iv) masking 0\% of the target history increases WER by $1.1$, as the model over-relies on text priors for subword completion and ignores the acoustic pathway. Conversely, excessive masking ($20$–$30$\%) destroys the linguistic context required for clean inference;
(v) $k=5$ matches the prompt-path resolution and adds less temporal detail ($+0.4$). $k=1$ gains only $0.2$ while tripling the key/value length of every cross-attention module, so $k=3$ offers the best accuracy--cost trade-off
