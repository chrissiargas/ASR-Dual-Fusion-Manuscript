...
\textbf{Dataset} & \textbf{Ours} & \textbf{Whisper} & \textbf{Vox}$^\dagger$ & \textbf{WER$_p$} & \textbf{CER} \\
\midrule
...
Logotypographia$^\ddagger$~\cite{digalakis2003logotypographia} & \textbf{5.6} & 10.3 & 9.4 & 5.6 & 1.6 \\
...
$^\dagger$~Taken from~\cite{damianos2025voxkrikriunifyingspeechlanguage}, which uses its own text normalization; indicative only.
$^\ddagger$~References are transcribed without accents, so accents are removed from both systems' output before scoring.
\end{table}
...
\begin{table}[h]
    ...
    \begin{tabular}{l | c c | c c}
        ...
        Yodas$^\dagger$~\cite{li2023yodas}         & 11.3 & \textbf{7.1} & 12.6 & 6.1 \\
        \bottomrule
    \end{tabular}%
    }
    \\
    \vspace{4pt}
    \raggedright \scriptsize
    $^\dagger$~Yodas references are pseudo-labels produced by Whisper-large-v3 itself, so this row scores us against the system that generated the references.
\end{table}
